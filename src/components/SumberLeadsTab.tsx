...
import { useSumberLeads, useCreateSumberLeads, useUpdateSumberLeads, useDeleteSumberLeads } from '@/hooks/useSumberLeads'
import { useForm } from 'react-hook-form'
import { zodResolver } from '@hookform/resolvers/zod'
import { sumberLeadsSchema, type SumberLeadsInput } from '@/lib/validations/sumber-leads'

export function SumberLeadsTab() {
  const [showModal, setShowModal] = useState(false)
  const [searchTerm, setSearchTerm] = useState('')
  const [showEditModal, setShowEditModal] = useState(false)
  const [editingSumberLeads, setEditingSumberLeads] = useState<any>(null)
  const [showDeleteModal, setShowDeleteModal] = useState(false)
  const [sumberLeadsToDelete, setSumberLeadsToDelete] = useState<any>(null)

  // Fetch sumber leads data
  const { data: sumberLeadsList = [], isLoading } = useSumberLeads()
  const createMutation = useCreateSumberLeads()
  const updateMutation = useUpdateSumberLeads()
  const deleteMutation = useDeleteSumberLeads()
  ...
  const onSubmit = async (data: SumberLeadsInput) => {
    await createMutation.mutateAsync(data)
    closeModal()
  }

  const openEditModal = (sumberLeads: any) => {
    setEditingSumberLeads(sumberLeads)
    setValueEdit('nama', sumberLeads.nama)
    setValueEdit('deskripsi', sumberLeads.deskripsi || '')
    setShowEditModal(true)
  }

  const closeEditModal = () => {
    setShowEditModal(false)
    setEditingSumberLeads(null)
    resetEdit()
  }

  const onEditSubmit = async (data: SumberLeadsInput) => {
    if (editingSumberLeads) {
      await updateMutation.mutateAsync({ id: editingSumberLeads.id, data })
      closeEditModal()
    }
  }

  const openDeleteModal = (sumberLeads: any) => {
    setSumberLeadsToDelete(sumberLeads)
    setShowDeleteModal(true)
  }

  const closeDeleteModal = () => {
    setShowDeleteModal(false)
    setSumberLeadsToDelete(null)
  }

  const handleDelete = async () => {
    if (sumberLeadsToDelete) {
      await deleteMutation.mutateAsync(sumberLeadsToDelete.id)
      closeDeleteModal()
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Edit Modal */}
      {showEditModal && editingSumberLeads && (
        <div
      ...
      {/* Delete Modal */}
      {showDeleteModal && sumberLeadsToDelete && (
        <div
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4">
            ...
            <div className="px-6 pb-6">
              <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
                <p className="text-slate-900 mb-2">Apakah Anda yakin ingin menghapus sumber leads ini?</p>
                <div className="bg-white border border-red-200 rounded-lg p-3">
                  <p className="font-semibold text-slate-900">{sumberLeadsToDelete.nama}</p>
      ...
      <div className="bg-white shadow-lg rounded-2xl border border-slate-200 overflow-hidden">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600">Kelola sumber leads untuk tracking asal prospek</p>
              </div>
            </div>
            <div className="text-sm text-slate-500">
              Total: <span className="font-medium text-slate-900">{filteredSumberLeads.length}</span> sumber leads
            </div>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {filteredSumberLeads.length > 0 ? (
                filteredSumberLeads.map((sumberLeads: any) => (
                  <TableRow key={sumberLeads.id} className="hover:bg-slate-50 border-b border-slate-100">
                    <TableCell className="py-4 px-6">
                      <div className="flex items-center gap-3">
                        <div className="h-10 w-10 bg-gradient-to-br from-indigo-500 to-indigo-600 rounded-full flex items-center justify-center text-white font-semibold text-sm">
                          {sumberLeads.nama.charAt(0).toUpperCase()}
                        </div>
                        <div>
                    ...
                    <TableCell className="py-4 px-6 text-slate-700">
                      {sumberLeads.deskripsi || '-'}
                    </TableCell>
                    <TableCell className="py-4 px-6 text-slate-700">
                      {new Date(sumberLeads.createdAt).toLocaleDateString('id-ID', {
                        day: 'numeric',
                        month: 'long',
                        year: 'numeric',
                      })}
        ...
        <div className="bg-slate-50 border-t border-slate-200 px-6 py-4">
          <div className="flex items-center justify-between text-sm text-slate-600">
            <div className="flex items-center gap-2">
              ...
              <span>
                Menampilkan {filteredSumberLeads.length > 0 ? '1' : '0'} hingga {filteredSumberLeads.length} dari{' '}
                {filteredSumberLeads.length} sumber leads
              </span>
            </div>
            {filteredSumberLeads.length > 0 && (
